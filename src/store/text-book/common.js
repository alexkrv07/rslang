const ActionType = {
  SET_PAGE: 'textbook/set-page',
  SET_GROUP: 'textbook/set-group',
  GET_WORD_LIST: 'textbook/get-word-list',
  GET_DIFFICULT_WORD_LIST: 'textbook/get-difficult-word-list',
  GET_LEARNED_WORD_LIST: 'textbook/get-learned-word-list',
  ADD_DIFFICULT_WORD: 'textbook/add-difficult-word',
  ADD_LEARNED_WORD: 'textbook/add-learned-word',
  DELETE_DIFFICULT_WORD: 'textbook/delete-difficult-word',
  DELETE_LEARNED_WORD: 'textbook/delete-learned-word',
  SET_PLAY_LIST: 'textbook/set-play-list',
  PLAY_PLAY_LIST: 'textbook/play-play-list',
  STOP_PLAY_LIST: 'textbook/stop-play-list',
};

export { ActionType };